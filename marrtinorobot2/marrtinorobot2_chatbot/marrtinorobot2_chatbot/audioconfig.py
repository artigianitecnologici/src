...
def list_vosk_models(directory="../models/vosk"):
    models = []
...
def create_config_file():
    ...
    for i, model in enumerate(available_vosk_models):
        print(f"[{i}] {model}")
    ...
    if vosk_choice.isdigit() and int(vosk_choice) < len(available_vosk_models):
        vosk_model_path = f"../models/vosk/{available_vosk_models[int(vosk_choice)]}"
    elif os.path.isdir(vosk_choice):
        vosk_model_path = vosk_choice
    else:
        print("Scelta non valida. Impostazione del modello predefinito '../models/vosk/vosk-model-it-0.22'.")
        vosk_model_path = "../models/vosk/vosk-model-small-it-0.22"

 #   print("\nSeleziona il modello Whisper:")
 #   whisper_model = input("Inserisci il nome del modello Whisper (tiny, base, small, medium, large): ").lower()
 #   if whisper_model not in ['tiny', 'base', 'small', 'medium', 'large']:
 #       print("Modello non valido. Uso il modello predefinito 'small'.")
  #      whisper_model = 'small'

    keyword = input("Inserisci la parola chiave da rilevare (esempio: 'ciao'): ")

    config = {
        "model_vosk_path": "../models/vosk/vosk-model-it-0.22",
...
#        "whisper_model": whisper_model,
    ...
    print("\nConfigurazione salvata in config.json")

    # Test dell'audio in input
    print("\nInizia a parlare per il test audio (5 secondi)...")
